...
it('renders the Select page on default with Loading state ', () => {
    const wrapper = mount(<SelectProject jiraHostURL='https://jira.atlassian.com/' session={{name: 'cookie', value: '123'}}/>);
    expect(wrapper.state().phase).toEqual('Loading');
});
it('renders the Select page with Ready state and list of projects when projects loaded', async () => {
    const mock = new MockAdapter(axios);
    mock.onGet().reply(200, projectsMock);
    const wrapper = mount(<SelectProject jiraHostURL='https://jira.atlassian.com/' session={{name: 'cookie', value: '123'}}/>);
    await tick();
    expect(wrapper.state().phase).toEqual('Ready');
    expect(Object.keys(wrapper.state().projects).length).toEqual(projectsMock.length);
});
it('renders the Select page with fail state and error message set when projects loaded', async () => {
    const mock = new MockAdapter(axios);
    mock.onGet().replyOnce(403, {
        "errorMessages": [
            "The user does not have access to view projects"
        ],
        "errors": {}
    });
    const wrapper = mount(<SelectProject jiraHostURL='https://jira.atlassian.com/' session={{name: 'cookie', value: '123'}}/>);
    await tick();
    expect(wrapper.state().phase).toEqual('Fail');
    expect(wrapper.state().errorMessage).toEqual('Response code: 403');
