...
describe("SingleIssueTypeSelector", () => {
  ...
  it("renders without crashing", () => {
    ...
    ReactDOM.render(<SingleIssueTypeSelector updateSelectedIssueType={() => {
    }} selectedIssueType={0} workflow={[]}/>, div);
    ReactDOM.unmountComponentAtNode(div);
  });

  it("updates the issue type correctly when an issue type button is clicked", () => {
    const mockFn = jest.fn();
    const wrapper = shallow(<SingleIssueTypeSelector updateSelectedIssueType={mockFn} selectedIssueType={0}
                                                     workflow={workflow}/>);
